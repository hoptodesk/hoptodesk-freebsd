...
pub const PRIVACY_MODE_IMPL_WIN_INLINE: &str = "privacy_mode_impl_inline";
...
lazy_static::lazy_static! {
    pub static ref DEFAULT_PRIVACY_MODE_IMPL: String = {
        #[cfg(windows)]
        {
            if !crate::platform::is_x64() {
                if win_exclude_from_capture::is_supported() {
                    PRIVACY_MODE_IMPL_WIN_INLINE.to_owned()
                } else {
                    "".to_owned()
                }
                ...
                PRIVACY_MODE_IMPL_WIN_VIRTUAL_DISPLAY.to_owned()
            } else {
                "".to_owned()
            }
        }
...
lazy_static::lazy_static! {
    static ref PRIVACY_MODE_CREATOR: Arc<Mutex<HashMap<&'static str, PrivacyModeCreator>>> = {
        ...
        #[cfg(windows)]
        {
            ...
            if !crate::platform::is_x64() && win_exclude_from_capture::is_supported() {
                map.insert(win_inline_privacy::PRIVACY_MODE_IMPL, |impl_key: &str| {
                    Box::new(win_inline_privacy::PrivacyModeImpl::new(impl_key))
                });
            }
        }
