...
// Storage for pending print jobs during printer selection dialog
lazy_static::lazy_static! {
    static ref PENDING_PRINT_JOBS: Mutex<HashMap<i32, Vec<u8>>> = Mutex::new(HashMap::new());
}

/// Store print data for later retrieval after user selects printer
pub fn store_print_job(id: i32, data: Vec<u8>) {
    PENDING_PRINT_JOBS.lock().unwrap().insert(id, data);
}

/// Retrieve and remove print data after printer is selected
pub fn take_print_job(id: i32) -> Option<Vec<u8>> {
    PENDING_PRINT_JOBS.lock().unwrap().remove(&id)
}
...
impl<T: InvokeUiSession> Session<T> {
    ...
    fn resolve_auto_keyboard_mode(&self) -> String {
        let peer_version = self.get_peer_version();
        let platform = self.peer_platform();
        if is_keyboard_mode_supported(&KeyboardMode::Map, peer_version, &platform) {
            return KeyboardMode::Map.to_string();
        }
        KeyboardMode::Legacy.to_string()
    }
    ...
    pub fn get_keyboard_mode(&self) -> String {
        ...
        if let Ok(mode) = keyboard_mode {
            if mode == KeyboardMode::Auto {
                return self.resolve_auto_keyboard_mode();
            }
            let peer_version = self.get_peer_version();
            let platform = self.peer_platform();
        ...
        self.resolve_auto_keyboard_mode()
    }

    pub fn get_raw_keyboard_mode(&self) -> String {
        let mode = self.lc.read().unwrap().keyboard_mode.clone();
        if mode.is_empty() {
    ...
    pub fn is_remote_printing_supported(&self) -> bool {
        self.lc.read().unwrap().is_remote_printing_supported()
    }
    ...
    pub fn is_switchsides_supported(&self) -> bool {
        crate::common::is_support_switchsides_num(self.lc.read().unwrap().version)
    }	
...
pub async fn io_loop<T: InvokeUiSession>(handler: Session<T>, round: u32) {
    ...
    #[cfg(target_os = "windows")]
    let _ole_guard = crate::SimpleCallOnReturn {
        b: true,
        f: Box::new(|| unsafe {
            // OleUninitialize is not exported by winapi 0.3.9's ole2 module,
            // so we declare it directly via FFI.
            extern "system" {
                fn OleUninitialize();
            }
            OleUninitialize();
...
//                                &token,
            ...
            if handler.args.len() != 3
                ...
                || port <= 0
            {
                handler.on_error("Invalid arguments, usage:<br><br> hoptodesk --port-forward remote-id listen-port remote-host remote-port");
            }
