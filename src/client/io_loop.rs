...
use crate::{
    ...
    ui_session_interface::{store_print_job, InvokeUiSession, Session},
};
...
impl ParsedPeerInfo {
    fn is_support_virtual_display(&self) -> bool {
        self.is_installed
            ...
            && (self.idd_impl == "hoptodesk_idd" || self.idd_impl == "amyuni_idd")
    }
}
...
impl<T: InvokeUiSession> Remote<T> {
    ...
    pub async fn io_loop(&mut self, _key: &str, _token: &str, round: u32) {
        ...
        self.handler.update_direct(None);
        self.handler.update_received(false);
                    ...
                    tokio::select! {
                        ...
                        _ = self.timer.tick() => {
                            ...
                            if !self.read_jobs.is_empty() {
                                let is_direct = self.handler.lc.read().unwrap().direct.unwrap_or(false);
                                if let Err(err) = fs::handle_read_jobs(&mut self.read_jobs, &mut peer, is_direct).await {
                        ...
                        _ = status_timer.tick() => {
                            ...
                            // Prevent integer overflow in speed calculation
                            // Use saturating operations and u64 for intermediate calculations
                            let speed_bps = if elapsed > 0 {
                                // Convert to u64 to prevent overflow, then back to safe range
                                let speed_u64 = (speed as u64).saturating_mul(1000) / (elapsed as u64);
                                // Cap at reasonable maximum to prevent display issues
                                std::cmp::min(speed_u64, u32::MAX as u64) as usize
                            } else {
                                0
                            };
                            
                            let speed_display = format!("{:.2}kB/s", speed_bps as f32 / 1024.0);

                            let fps = self.video_threads.iter().map(|(k, v)| {
                                // Correcting the inaccuracy of status_timer
                                // Also prevent overflow here
                                let frame_count = *v.frame_count.read().unwrap() as i64;
                                let fps_calc = if elapsed > 0 {
                                    ((frame_count * 1000) / elapsed as i64).min(i32::MAX as i64) as i32
                                } else {
                                    0
                                };
                                (k.clone(), fps_calc)
                ...
                if is_remote {
                    ...
                    match fs::TransferJob::new_read(
                        ...
                        Ok(mut job) => {
                            ...
                            job.direct = self.handler.lc.read().unwrap().direct.unwrap_or(false);
    ...
    fn update_jobs_status(&mut self) {
        ...
        if elapsed >= 1000 {
            ...
            for job in self.write_jobs.iter() {
                // Skip printer jobs - they don't need progress updates in UI
                if job.r#type == fs::JobType::Printer {
                    continue;
                }
                Self::update_job_status(
                    job,
                                ...
                                fs::JobType::Printer => {
                                    if let Some(err) = err {
                                        ...
                                        if let Some(data) = printer_data {
                                            ...
                                            let auto_print = LocalConfig::get_option(
                                                keys::OPTION_PRINTER_ALLOW_AUTO_PRINT
                                            ) == "Y";
                                            let saved_printer = LocalConfig::get_option(
                                                keys::OPTION_PRINTER_SELECTED_NAME
                                            );

                                            if auto_print && !saved_printer.is_empty() {
                                                // Auto-print to saved printer
                                                let printer_name = saved_printer;
                                                std::thread::spawn(move || {
                                                    if let Err(e) =
                                                ...
                                                let job_id = d.id;
                                                let path = format!("print_job_{}.pdf", job_id);
                                                store_print_job(job_id, data);
                                                self.handler.printer_request(job_id, path);
                                            }
                                        }
                                    }
                                }
                ...
                Some(message::Union::FileAction(action)) => match action.union {
                    Some(file_action::Union::Send(_s)) => match _s.file_type.enum_value() {
                        ...
                        Ok(file_transfer_send_request::FileType::Printer) => {
                            ...
                            // The job uses a memory buffer instead of a file
                            let to = fs::DataSource::MemoryCursor(std::io::Cursor::new(Vec::new()));
                            self.write_jobs.push(fs::TransferJob::new_write(
                                ...
                                true, // is_remote
                                Vec::new(),
                                false, // no overwrite detection needed
                            ));
                            log::info!("Created printer write job: id={}", _s.id);
                        }
                        _ => {}
                    },
    ...
    async fn handle_back_msg_privacy_mode(
        ...
        match state {
            ...
            back_notification::PrivacyModeState::PrvOnSucceeded => {
                self.handler
                    .msgbox("custom-nocancel", "Privacy Mode", "Privacy Mode started.", "");
            ...
            back_notification::PrivacyModeState::PrvOffSucceeded => {
                self.handler
                    .msgbox("custom-nocancel", "Privacy Mode", "Privacy Mode turned off.", "");
                self.update_privacy_mode(impl_key, false);
            }
            back_notification::PrivacyModeState::PrvOffByPeer => {
                self.handler
                    .msgbox("custom-error", "Privacy Mode", "The peer has exited from Privacy Mode.", "");
                self.update_privacy_mode(impl_key, false);
            }
