...
impl RendezvousMediator {
    ...
    pub async fn start_all() {
        ...
        crate::server::set_incoming_server(&server);
        log::info!("Checking NAT type...");
        ...
        loop {
            ...
            if Config::get_option("stop-service").is_empty() {
                ...
                log::info!("Fetching rendezvous servers...");
				if let Some(servers) = Config::get_rendezvous_servers().await {
                    log::info!("Got rendezvous servers: {:?}", servers);
                    ...
                    log::warn!("get_rendezvous_servers returned None");
                }
            } else {
                log::info!("stop-service option is set, not connecting");
            }
        ...
        if let Ok(mut file) = fs::File::open(&Config::path("TeamID.toml")) {
            ...
            if let Err(e) = file.read_to_string(&mut teamid) {
                    ...
                    .await
                {
                    ...
                    Err(e) => log::warn!("Initial team check-in failed: {}", e),
                }
            }
        }
                                ...
                                Ok(Ok(_)) => {
                                    ...
                                    tokio::spawn(async move {
                                        if let Err(_error) = crate::server::accept(listener, server_clone, true, false).await {
                                            //log::error!("RendezvousMediator: accept() failed: {:?}", error);
                                        } else {
                                            //log::info!("RendezvousMediator: Successfully accepted connection (or timed out) for peer.");
                                        }
                                        });
