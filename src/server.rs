...
// Stub camera module for iOS
#[cfg(target_os = "ios")]
mod camera {
    pub const PRIMARY_CAMERA_IDX: usize = 0;
    pub fn primary_camera_exists() -> bool { false }
    pub struct Cameras;
    impl Cameras {
        pub fn all_info() -> Option<Vec<hbb_common::message_proto::DisplayInfo>> { None }
        pub fn get_camera_resolution(_idx: usize) -> hbb_common::ResultType<hbb_common::message_proto::Resolution> {
            hbb_common::anyhow::bail!("Camera not supported on iOS")
        }
        pub fn get_sync_cameras() -> Vec<hbb_common::message_proto::DisplayInfo> { vec![] }
        pub fn get_capturer(_idx: usize) -> hbb_common::ResultType<Box<dyn scrap::TraitCapturer>> {
            hbb_common::anyhow::bail!("Camera capture not supported on iOS")
        }
    }
}
...
lazy_static::lazy_static! {
    ...
    static ref INCOMING_SERVER: Mutex<Option<ServerPtr>> = Mutex::new(None);
}
...
pub fn set_incoming_server(server: &ServerPtr) {
    *INCOMING_SERVER.lock().unwrap() = Some(server.clone());
}

/// Send close_reason to all connected remote peers before the process exits,
/// so they do not attempt to reconnect.
pub fn graceful_shutdown_server() {
    if let Some(server) = INCOMING_SERVER.lock().unwrap().as_ref() {
        let count = server.read().unwrap().connections.len();
        if count > 0 {
            log::info!("[graceful_shutdown_server] Closing {} connection(s)", count);
...
pub fn new() -> ServerPtr {
    ...
    // display_service monitors for display changes and pushes updated PeerInfo
    // to remote peers. Needed on iOS so the remote learns the real screen size
    // once the broadcast extension starts (initial size may be 0x0).
    ...
    match timeout(CONNECT_TIMEOUT, stream.next()).await? {
        Some(Ok(bytes)) => {
            if let Ok(msg_in) = Message::parse_from_bytes(&bytes) {
                ...
                log::error!("SERVER_TRACE: next_message_for_handshake - Failed to parse bytes into Message.");
            }
        }
        Some(Err(e)) => {
            log::error!("SERVER_TRACE: next_message_for_handshake - stream.next() error: {:?}", e);
        }
        None => {
            log::error!("SERVER_TRACE: next_message_for_handshake - stream.next() returned None (stream closed).");
        }
    }
...
pub async fn create_tcp_connection(
    ...
    log::info!("Creating tcp connection from addr: {:?}", addr);
        ...
        if is_direct_initial_public_key_request(&msg_in) {
            ...
            if let Some(message::Union::PublicKey(pk)) = msg_in.union {
                match process_client_public_key_message(pk, &our_sk_b, &mut security_numbers)? {
                    ...
                    None => log::info!("Client sent empty PublicKey, continuing without encryption"),
                }
            }
            else {
    ...
    Connection::start(addr,stream,id,Arc::downgrade(&server),security_numbers).await;
    Ok(())
}
...
pub async fn start_server(is_server: bool, no_server: bool) {
    ...
    log::info!("start_server(is_server={}, no_server={})", is_server, no_server);
    if is_server {
        ...
        log::info!("IPC started, now starting RendezvousMediator...");
        ...
        log::info!("About to call RendezvousMediator::start_all()");
        crate::RendezvousMediator::start_all().await;
        log::info!("RendezvousMediator::start_all() returned");
    } else {
        log::info!("Trying IPC connect (is_server=false)...");
        match crate::ipc::connect(1000, "").await {
            Ok(mut conn) => {
                log::info!("IPC connect succeeded — another server instance is running");
            ...
            Err(err) => {
                log::info!("IPC connect failed: {:?}, no_server: {}", err, no_server);
                if no_server {
...
pub fn on_printer_data(data: Vec<u8>) {
    ...
    // Send to connection manager via IPC
    // The cm will then forward to all active connections
    std::thread::spawn(move || {
        let rt = tokio::runtime::Runtime::new().unwrap();
        rt.block_on(async {
            match crate::ipc::connect(1000, "_cm").await {
                Ok(mut conn) => {
                    if let Err(e) = conn.send(&crate::ipc::Data::PrinterData(data)).await {
                        log::error!("Failed to send PrinterData to cm: {}", e);
                    } else {
                        log::info!("Successfully sent PrinterData to cm");
                    }
                }
                Err(e) => {
                    log::error!("Failed to connect to cm for PrinterData: {}", e);
                }
            }
        });
    });
