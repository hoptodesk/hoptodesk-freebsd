...
pub fn new(source: VideoSource, idx: usize) -> GenericService {
    ...
    GenericService::run(&vs, run_wrapper);
    vs.sp
}

/// Wrapper around run() to capture all errors for iOS diagnostics
fn run_wrapper(vs: VideoService) -> ResultType<()> {
    let result = run(vs);
    #[cfg(target_os = "ios")]
    if let Err(ref e) = result {
        scrap::ios::ffi::diag_video_svc_error(&format!("run: {}", e));
    }
    result
}
...
fn get_capturer_monitor(
    ...
    // On iOS (and Android), the screen size may not be available yet if the broadcast
    // extension hasn't started or hasn't sent its first frame. Wait and retry rather
    // than creating an encoder with 0x0 dimensions which would fail.
    #[cfg(any(target_os = "ios", target_os = "android"))]
    if display.width() == 0 || display.height() == 0 {
        #[cfg(target_os = "ios")]
        scrap::ios::ffi::nslog("[video_service] Display has 0x0 dimensions, waiting for screen capture to start");
        bail!("Display has 0x0 dimensions, waiting for screen capture to start");
    }
...
fn run(vs: VideoService) -> ResultType<()> {
    #[cfg(target_os = "ios")]
    scrap::ios::ffi::diag_video_svc_started();
    log::info!("[video_service] run() started for display_idx={}, source={:?}", vs.idx, vs.source);
    ...
    let mut c = match get_capturer(vs.source, display_idx, last_portable_service_running) {
        ...
        Err(e) => {
            #[cfg(target_os = "ios")]
            scrap::ios::ffi::diag_video_svc_error(&format!("get_capturer: {}", e));
            return Err(e);
        }
    };
    #[cfg(windows)]
    ...
    let (mut encoder, encoder_cfg, codec_format, use_i444, recorder) = match setup_encoder(
        ...
        Err(err) => {
            ...
            #[cfg(target_os = "ios")]
            scrap::ios::ffi::diag_video_svc_error(&format!("encoder1: {}", err));
            ...
            match setup_encoder(
                ...
                Err(e) => {
                    #[cfg(target_os = "ios")]
                    scrap::ios::ffi::diag_video_svc_error(&format!("encoder2: {}", e));
                    return Err(e);
                }
            }
        }
