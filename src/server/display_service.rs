...
fn run(sp: EmptyExtraFieldService) -> ResultType<()> {
    #[cfg(target_os = "ios")]
    scrap::ios::ffi::nslog("[display_service] run() started");
    log::info!("[display_service] run() started");
    #[cfg(any(target_os = "ios", target_os = "android"))]
    {
        if let Ok(displays) = try_get_displays() {
            for (i, d) in displays.iter().enumerate() {
                #[cfg(target_os = "ios")]
                scrap::ios::ffi::nslog(&format!("[display_service] initial display[{}]: {}x{} origin=({},{}) name={}",
                    i, d.width(), d.height(), d.origin().0, d.origin().1, d.name()));
                log::info!("[display_service] initial display[{}]: {}x{} origin=({},{}) name={}",
                    i, d.width(), d.height(), d.origin().0, d.origin().1, d.name());
            }
        }
    }
    while sp.ok() {
        ...
        if let Some(msg_out) = check_get_displays_changed_msg() {
            sp.send(msg_out);
            #[cfg(target_os = "ios")]
            scrap::ios::ffi::nslog("[display_service] Displays changed — sending update to peers");
...
pub(super) fn get_original_resolution(
    ...
    let is_rustdesk_virtual_display =
        crate::virtual_display_manager::hoptodesk_idd::is_virtual_display(&display_name);
    #[cfg(not(windows))]
