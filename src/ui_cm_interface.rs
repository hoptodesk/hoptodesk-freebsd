...
#[cfg(not(any(target_os = "android", target_os = "ios")))]
impl Client {
    pub fn get_tx(&self) -> &UnboundedSender<Data> {
        &self.tx
    }
}
...
lazy_static::lazy_static! {
    pub static ref CLIENTS: RwLock<HashMap<i32, Client>> = Default::default();
}
...
#[cfg(not(any(target_os = "android", target_os = "ios")))]
pub fn close_all_clients() {
    let clients = CLIENTS.read().unwrap();
    //let total = clients.len();
    let mut _sent = 0;
    for (_id, client) in clients.iter() {
        if !client.disconnected {
            match client.tx.send(Data::Close) {
                Ok(_) => {
                    _sent += 1;
                }
                Err(_e) => {
                    //log::error!("[close_all_clients] Failed to send Data::Close to client #{}: {:?}", id, e);
                }                
            }
        } 
    }
            ...
            tokio::select! {
                res = self.stream.next() => {
                    match res {
                        Ok(Some(data)) => {
                            match data {
                                Data::Login{id, is_file_transfer, is_view_camera, is_terminal, port_forward, peer_id, name, authorized, keyboard, clipboard, audio, file, file_transfer_enabled: _file_transfer_enabled, restart, recording, block_input, from_switch, is_invite, password_to_connect_to_inviter, security_numbers, avatar_image} => {
                                    // Create the client struct from the login data.
                                    let client = crate::ui_cm_interface::Client {
                                        id, authorized, disconnected: false, is_file_transfer, is_view_camera, is_terminal,
                                        port_forward: port_forward.clone(), name: name.clone(), peer_id: peer_id.clone(),
                                    ...
                                    // Add the connection to the UI. TIScript will handle rendering based on the `is_invite` flag.
                                    self.cm.add_connection(&client, security_numbers, avatar_image);
                                    ...
                                    // For normal (non-invite) connections, break to the main I/O loop.
                                    // For invites, the loop continues here, keeping the CM window alive.
                                    if !is_invite {
                                    #[cfg(target_os = "windows")]
                                    {
                                        self.file_transfer_enabled = _file_transfer_enabled;
                                            if ContextSend::is_enabled() {
                                                log::debug!("Clipboard is enabled, sending MonitorReady");
                                                allow_err!(
                                                    self.stream
                                                        .send(&Data::ClipboardFile(clipboard::ClipboardFile::MonitorReady))
                                                        .await
                                ...
                                // Keep other handlers from the user's code
                                #[cfg(not(any(target_os = "android", target_os = "ios")))]
                                ...
                                Data::PrinterData(data) => {
                                    ...
                                    for (id, client) in clients.iter() {
                                        if client.authorized && !client.disconnected {
                                            if let Err(e) = client.tx.send(Data::PrinterData(data.clone())) {
                                                log::error!("[cm] Failed to forward PrinterData to client {}: {}", id, e);
                                            } else {
                                                log::info!("[cm] Forwarded PrinterData to client {}", id);
                                                sent_count += 1;
                                            }
                                        }
                                    }
                                    log::info!("[cm] PrinterData forwarded to {} connections", sent_count);
                                }
                                _ => {
                ...
                Some(data) = self.rx.recv() => {
                    if matches!(&data, Data::Close) {
                        //log::info!("[IpcTaskRunner] Received Data::Close on rx for conn_id={}, forwarding via IPC stream", self.conn_id);
                    }
                    if let Err(_e) = self.stream.send(&data).await {
                        //log::error!("[IpcTaskRunner] error sending data via IPC stream for conn_id={}: {}", self.conn_id, e);
                        break;
                    }
                    if matches!(&data, Data::Close) {
                        //log::info!("[IpcTaskRunner] Successfully forwarded Data::Close via IPC stream for conn_id={}", self.conn_id);
                    }
                    match &data {
...
pub async fn start_listen<T: InvokeUiCM>(
    ...
    loop {
        match rx.recv().await {
                ...
                cm.add_connection(&client, security_numbers, avatar_image);
...
async fn handle_fs(
    ...
    match fs {
        ...
        ipc::FS::NewWrite {
            ...
            // Create empty directories upfront (Dir marker entries have trailing "/")
...
pub fn send_raw(msg: hbb_common::message_proto::Message, tx: &hbb_common::tokio::sync::mpsc::UnboundedSender<crate::ipc::Data>) {
    match msg.write_to_bytes() {
        Ok(bytes) => {
            if let Err(e) = tx.send(crate::ipc::Data::RawMessage(bytes)) {
                log::error!("Failed to send message via ipc tx: {}", e);
        }
        }
        Err(e) => {
            log::error!("Failed to serialize message for sending: {}", e);
        }
    }
}
...
pub fn get_clients_lock() -> Result<std::sync::RwLockReadGuard<'static, HashMap<i32, Client>>, String> {
    CLIENTS.read().map_err(|e| format!("Failed to acquire read lock on CLIENTS: {:?}", e))
}
