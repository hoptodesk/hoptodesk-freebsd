...
fn initialize(app_dir: &str) {
    ...
    #[cfg(target_os = "ios")]
    {
        ...
        crate::common::test_rendezvous_server();
        crate::common::test_nat_type();
        // Start with service stopped; main_start_service() will clear this
        // when the user starts screen sharing via the broadcast picker.
        config::Config::set_option("stop-service".into(), "Y".into());
        std::thread::spawn(move || crate::server::start_server(true));
    }
    #[cfg(not(any(target_os = "android", target_os = "ios")))]
    {
...
pub fn is_custom_client() -> SyncReturn<bool> {
    SyncReturn(get_app_name() != "HopToDesk")
}
