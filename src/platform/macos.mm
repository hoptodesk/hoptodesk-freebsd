...
extern "C" bool MacSetMode(CGDirectDisplayID display, uint32_t width, uint32_t height, bool tryHiDPI)
{
    ...
    // Skip if already at requested resolution (prevents macOS from minimizing maximized windows)
    if (CGDisplayModeGetWidth(currentMode) == width && CGDisplayModeGetHeight(currentMode) == height) {
        CGDisplayModeRelease(currentMode);
        return true;
    }
