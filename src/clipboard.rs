...
const RUSTDESK_CLIPBOARD_OWNER_FORMAT: &'static str = "dyn.com.hoptodesk.owner";
...
pub fn is_file_url_set_by_rustdesk(url: &Vec<String>) -> bool {
    ...
    url.iter()
        .next()
        .map(|s| {
            for prefix in &["file:///tmp/.hoptodesk_", "//tmp/.hoptodesk_"] {
        ...
        if let Ok(data) = self.get_formats(&[ClipboardFormat::FileUrl]) {
            ...
            if !urls.is_empty() {
                ...
                let clear_holder_text = if is_kde_x11 {
                    "Placeholder to clear the file clipbard"
                } else {
                    ""
                }
                .to_string();
