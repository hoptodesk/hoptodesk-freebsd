...
impl ClientInternal {
    ...
    async fn listen(&mut self) -> Result<()> {
        ...
        self.on_connection_bound = Some(rx);
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                //log::debug!("received {} bytes of tcp from {}", n, from);

                let realm_str = {
                    realm.lock().unwrap().to_string()
                };
    ...
    async fn wait_new_connection(&mut self) -> Option<MaybeTlsStream> {
        self.on_connection_bound.as_mut()?.recv().await
    }
    ...
    async fn handle_inbound(
        ...
        if is_message(data) {
            ClientInternal::handle_stun_message(tr_map, read_ch_tx, data, from, username, password, realm, tls_config, nonce, on_connection_bound, socks5_proxy).await
    ...
    async fn handle_stun_message(
        ...
        if msg.typ.class == CLASS_INDICATION {
            if msg.typ.method == METHOD_DATA {
                ...
                client.integrity = MessageIntegrity::new_long_term_integrity(
                    client.username.text.clone(),
                    client.realm.lock().unwrap().to_string(),
                ...
                msg.build(&[
                    ...
                    Box::new(client.username.clone()),
                    Box::new(client.realm.lock().unwrap().clone()),
                    Box::new(nonce.clone()),
                    Box::new(client.integrity.clone()),
                    Box::new(FINGERPRINT),
                ])?;
        
                client.conn.send(&msg.raw).await.unwrap();
                let mut buff = [0; MAX_DATA_BUFFER_SIZE];
                client.conn.recv(&mut buff).await.unwrap();
    ...
    async fn allocate(&mut self) -> Result<RelayConnConfig> {
        ...
        *self.nonce.lock().await = nonce.clone();

        *self.realm.lock().unwrap() = Realm::get_from_as(&res, ATTR_REALM)?;

        self.integrity = MessageIntegrity::new_long_term_integrity(
            self.username.text.clone(),
            self.realm.lock().unwrap().to_string(),
            self.password.clone(),
        );
        ...
        msg.build(&[
            ...
            Box::new(self.realm.lock().unwrap().clone()),
            Box::new(nonce),
        ...
        Ok(RelayConnConfig {
            ...
            nonce: self.nonce.lock().await.clone(),
            lifetime: lifetime.0,
