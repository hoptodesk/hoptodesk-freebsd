...
impl FrameRaw {
    ...
    fn update(&mut self, data: *mut u8, len: usize) {
        if !self.enable || data.is_null() || len == 0 {
            return;
        }

        // SAFE: Copy while Java buffer is valid (during JNI call)
        self.data.resize(len, 0);
        unsafe {
            std::ptr::copy_nonoverlapping(data, self.data.as_mut_ptr(), len);
        }
...
pub extern "system" fn Java_ffi_FFI_onAudioFrameUpdate(
    ...
    if let Ok(data) = env.get_direct_buffer_address(&jb) {
        if let Ok(len) = env.get_direct_buffer_capacity(&jb) {
            if let Ok(mut raw) = AUDIO_RAW.lock() {
                raw.update(data, len);
            }
        }
    }
