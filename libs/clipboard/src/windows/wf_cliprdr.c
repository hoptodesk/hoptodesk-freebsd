													...
													ULONG *pcbRead)
{
	...
	*pcbRead = 0;
	...
	{
		ULONG copy_size = clipboard->req_fsize;
		if (copy_size > cb)
			copy_size = cb;
		CopyMemory(pv, clipboard->req_fdata, copy_size);
		free(clipboard->req_fdata);
		clipboard->req_fdata = NULL;
		clipboard->req_fsize = copy_size;
	}
	...
	if (instance)
	{
		...
		if (iStream->lpVtbl)
		{
			...
			if (((instance->m_Dsc.dwFlags & FD_FILESIZE) == 0) && !isDir)
			{
				...
				else if (clipboard->req_fdata != NULL)
				{
					free(clipboard->req_fdata);
					clipboard->req_fdata = NULL;
				}
												...
												void *data)
{
	...
	return instance;
error:
	/* CliprdrDataObject_Delete handles freeing lpVtbl, m_pFormatEtc,
	 * m_pStgMedium, m_pStream, and the instance itself. */
	if (instance)
	{
		CliprdrDataObject_Delete(instance);
	}
	else if (iDataObject && iDataObject->lpVtbl)
	{
		/* instance was never allocated, but iDataObject->lpVtbl was */
...
static BOOL map_ensure_capacity(wfClipboard *clipboard)
{
	...
	if (clipboard->map_size >= clipboard->map_capacity)
	{
		...
		old_capacity = clipboard->map_capacity;
		...
		/* Zero-initialize the newly allocated entries to prevent use of uninitialized pointers */
		memset(new_map + old_capacity, 0, sizeof(formatMapping) * (new_size - old_capacity));
										  ...
										  const CLIPRDR_FORMAT_LIST *formatList)
{
	...
	{
		/* Ensure capacity BEFORE writing to format_mappings[i] */
		if (!map_ensure_capacity(clipboard))
		{
			rc = ERROR_INTERNAL_ERROR;
			goto exit_format_list;
		}
...
exit:
	if (clipboard && clipboard->context && clipboard->context->ClientFormatDataResponse && formatDataRequest)
	{
		if (rc == ERROR_SUCCESS)
		{
									   ...
									   const CLIPRDR_FORMAT_DATA_RESPONSE *formatDataResponse)
{
	...
	do
	{
		...
		if (formatDataResponse->requestedFormatData && formatDataResponse->dataLen > 0)
		{
 ...
 * Function description
 *
 * Serves file content requests from the remote side. When the local user copies
 * files and the remote tries to paste, this function provides the file data.
 *
 * Strategy: prefer direct filesystem reads (CreateFile/ReadFile) over COM IStream.
 * The filesystem path is simpler, doesn't require COM apartment message pumping,
 * and avoids stale COM proxy issues. IStream is only used as a fallback when
 * we don't have direct filesystem access to the file (e.g., virtual shell items).
 *
 * @return 0 on success, otherwise a Win32 error code
 */
static UINT
										...
										const CLIPRDR_FILE_CONTENTS_REQUEST *fileContentsRequest)
{
	...
	DEBUG_CLIPRDR("server_file_contents_request: enter");

	if (!context || !fileContentsRequest)
	{
		DEBUG_CLIPRDR("server_file_contents_request: null context or request");
		rc = ERROR_INTERNAL_ERROR;
		goto exit;
	}
	...
	if (!clipboard)
	{
		DEBUG_CLIPRDR("server_file_contents_request: null clipboard");
		rc = ERROR_INTERNAL_ERROR;
		goto exit;
	}

	DEBUG_CLIPRDR("server_file_contents_request: conn_id=%u, stream_id=%u, list_index=%u, dw_flags=%u, cb_requested=%u, nFiles=%u",
		fileContentsRequest->connID, fileContentsRequest->streamId,
		fileContentsRequest->listIndex, fileContentsRequest->dwFlags,
		fileContentsRequest->cbRequested, (unsigned int)clipboard->nFiles);
	...
	if (is_set_by_instance(clipboard) || is_file_descriptor_from_remote()) {
		DEBUG_CLIPRDR("server_file_contents_request: clipboard set by instance or descriptor from remote, rejecting");
		rc = ERROR_INTERNAL_ERROR;
		goto exit;
	}
	...
	if (cbRequested == 0)
		cbRequested = 1; /* avoid calloc(1,0) ambiguity */

	pData = (BYTE *)calloc(1, cbRequested);

	if (!pData)
	{
		DEBUG_CLIPRDR("server_file_contents_request: calloc(%u) failed", cbRequested);
		rc = ERROR_INTERNAL_ERROR;
		goto exit;
	}

	/*
	 * ===== PREFERRED PATH: Direct filesystem read =====
	 *
	 * When clipboard->nFiles > 0 and the file_names array is populated
	 * (set by wf_cliprdr_server_format_data_request for FileGroupDescriptorW),
	 * read the file directly from disk. This avoids COM IStream issues entirely.
	 */
	if (clipboard->nFiles > 0 &&
		clipboard->file_names &&
		clipboard->fileDescriptor &&
		fileContentsRequest->listIndex < (UINT32)clipboard->nFiles &&
		clipboard->file_names[fileContentsRequest->listIndex] != NULL)
	{
		DEBUG_CLIPRDR("server_file_contents_request: using filesystem path for list_index=%u", fileContentsRequest->listIndex);

		if (fileContentsRequest->dwFlags == FILECONTENTS_SIZE)
		{
			if (clipboard->fileDescriptor[fileContentsRequest->listIndex])
			{
				*((UINT32 *)&pData[0]) =
				...
				*((UINT32 *)&pData[4]) =
				...
				uSize = cbRequested;
				bHandled = TRUE;
				DEBUG_CLIPRDR("server_file_contents_request: filesystem SIZE ok, size_low=%u, size_high=%u",
					clipboard->fileDescriptor[fileContentsRequest->listIndex]->nFileSizeLow,
					clipboard->fileDescriptor[fileContentsRequest->listIndex]->nFileSizeHigh);
			}
			else
			{
				DEBUG_CLIPRDR("server_file_contents_request: fileDescriptor[%u] is NULL", fileContentsRequest->listIndex);
			}
		}
		else if (fileContentsRequest->dwFlags == FILECONTENTS_RANGE)
		{
			...
			if (clipboard->nFiles > 0 &&
				...
				if (clipboard->context && clipboard->context->HandleClipboardFiles)
			...
			{
				bHandled = TRUE;
				DEBUG_CLIPRDR("server_file_contents_request: filesystem RANGE ok, read %u bytes", uSize);
			}
			else
			{
				DEBUG_CLIPRDR("server_file_contents_request: filesystem RANGE failed, will try IStream fallback");
			}
		}
	}

	/*
	 * ===== FALLBACK PATH: COM IStream =====
	 *
	 * Only used when the filesystem path above didn't handle the request
	 * (e.g., virtual shell items, or file_names not populated yet).
	 * NOTE: We do NOT cache streams across calls. Each call gets a fresh stream
	 * and releases it at the end. This avoids stale COM proxy crashes that occur
	 * when streams are cached but the parent data object or COM apartment state
	...
	if (!bHandled)
	{
		...
		DEBUG_CLIPRDR("server_file_contents_request: trying IStream fallback path");

		hRet = OleGetClipboard(&pDataObj);
		if (FAILED(hRet))
		{
			DEBUG_CLIPRDR("server_file_contents_request: OleGetClipboard failed with 0x%08lX", hRet);
			rc = ERROR_INTERNAL_ERROR;
			goto exit;
		}
		...
		/* Try to get an IStream for this file */
		{
			LPENUMFORMATETC pEnumFormatEtc = NULL;
			ULONG CeltFetched;
			FORMATETC vFormatEtc2;

			hRet = IDataObject_EnumFormatEtc(pDataObj, DATADIR_GET, &pEnumFormatEtc);
			if (hRet == S_OK && pEnumFormatEtc)
			{
				do
				{
					...
					if (hRet == S_OK)
					{
						if (vFormatEtc2.cfFormat == RegisterClipboardFormat(CFSTR_FILECONTENTS))
						{
							...
							if (hRet == S_OK && vStgMedium.pstm)
							{
								pStream = vStgMedium.pstm;
								DEBUG_CLIPRDR("server_file_contents_request: IStream obtained");
							}
							else
							{
								DEBUG_CLIPRDR("server_file_contents_request: IDataObject_GetData for FILECONTENTS failed: 0x%08lX", hRet);
							}
							break;
						}
					}
			...
			{
				DEBUG_CLIPRDR("server_file_contents_request: EnumFormatEtc failed: 0x%08lX", hRet);
			}
		}
		...
		if (pStream)
		{
			if (fileContentsRequest->dwFlags == FILECONTENTS_SIZE)
			{
				...
				hRet = IStream_Stat(pStream, &vStatStg, STATFLAG_NONAME);
				if (hRet == S_OK)
				{
					...
					*((UINT32 *)&pData[4]) = vStatStg.cbSize.HighPart;
					uSize = cbRequested;
					bHandled = TRUE;
					DEBUG_CLIPRDR("server_file_contents_request: IStream SIZE ok");
				}
				else
				{
					DEBUG_CLIPRDR("server_file_contents_request: IStream_Stat failed: 0x%08lX", hRet);
				}
			}
			else if (fileContentsRequest->dwFlags == FILECONTENTS_RANGE)
			{
				...
				hRet = IStream_Seek(pStream, dlibMove, STREAM_SEEK_SET, &dlibNewPosition);
				if (SUCCEEDED(hRet))
				{
					hRet = IStream_Read(pStream, pData, cbRequested, (PULONG)&uSize);
					if (SUCCEEDED(hRet))
					{
						bHandled = TRUE;
						DEBUG_CLIPRDR("server_file_contents_request: IStream RANGE ok, read %u bytes", uSize);
					}
					else
					{
						DEBUG_CLIPRDR("server_file_contents_request: IStream_Read failed: 0x%08lX", hRet);
					}
				}
				else
				{
					DEBUG_CLIPRDR("server_file_contents_request: IStream_Seek failed: 0x%08lX", hRet);
				}
			}

			/* Always release the stream - do NOT cache it across calls.
			 * Caching streams from OLE clipboard data objects is fragile because:
			 * 1) The parent data object may be released, invalidating the stream
			 * 2) The COM apartment may not have a message pump for proper marshaling
			 * 3) Intermediate OleGetClipboard calls can change internal COM state */
			if (vStgMedium.pUnkForRelease)
			{
				ReleaseStgMedium(&vStgMedium);
			}
			else
			{
		...
		{
			DEBUG_CLIPRDR("server_file_contents_request: no IStream available from clipboard");
		}
	...
	if (bHandled)
		...
		DEBUG_CLIPRDR("server_file_contents_request: request not handled, returning error");
...
exit:
	...
	if (clipboard && fileContentsRequest)
	{
		sRc =
	...
	DEBUG_CLIPRDR("server_file_contents_request: exit, rc=%u, uSize=%u", rc, uSize);
	return rc;
}
										 ...
										 const CLIPRDR_FILE_CONTENTS_RESPONSE *fileContentsResponse)
{
	...
	do
	{
		...
		if (fileContentsResponse->requestedData && fileContentsResponse->cbRequested > 0)
		{
...
BOOL wf_cliprdr_uninit(wfClipboard *clipboard, CliprdrClientContext *cliprdr)
{
	...
	/* Release OLE clipboard ownership before cleanup.
	 * Without this, the clipboard stays locked after the session ends
	 * because OleSetClipboard() was called but never flushed. */
