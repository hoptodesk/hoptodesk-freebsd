            ...
            match entry.entry_type.enum_value() {
                ...
                Ok(FileType::Dir) => {
                    if let Ok(mut tmp) = read_dir_recursive(
                        ...
                        if tmp.is_empty() {
                            // Empty directory — add marker so it gets created on remote
                            let mut dir_entry = FileEntry::new();
                            dir_entry.name = get_string(&prefix.join(&entry.name)) + "/";
                            dir_entry.entry_type = FileType::Dir.into();
                            dir_entry.size = 0;
...
impl TransferJob {
    ...
    pub fn set_files(&mut self, files: Vec<FileEntry>) {
        ...
        if let DataSource::FilePath(p) = &self.data_source {
            for f in &files {
                if f.name.ends_with('/') {
                    let dir_path = Self::join(p, &f.name);
    ...
    pub async fn write(&mut self, block: FileTransferBlock) -> ResultType<()> {
        ...
        match &self.data_source {
            DataSource::FilePath(p) => {
                ...
                // Empty directory marker — create dir and skip file handling
                if self.files[file_num].name.ends_with('/') {
                    let dir_path = Self::join(p, &self.files[file_num].name);
                    std::fs::create_dir_all(&dir_path).ok();
        ...
        // Flush/sync written data periodically to prevent OS buffer accumulation
        // but avoid doing it on every single block as that kills small-file throughput.
        if let DataSource::FilePath(_) = &self.data_source {
            let file_num = block.file_num as usize;
            if file_num < self.files.len() {
                let entry = &self.files[file_num];
                if let Some(DataStream::FileStream(file)) = self.data_stream.as_mut() {
                    let file_complete = self.finished_size >= entry.size;
                    // Flush on file completion or periodically; use larger interval on direct connections
                    let flush_interval: u64 = if self.direct { 2 * 1024 * 1024 } else { 512 * 1024 };
                    if file_complete || self.transferred % flush_interval < block.data.len() as u64 {
                        file.flush().await?;
                    }
                }
            }
    ...
    pub async fn read(&mut self, stream: &mut Stream) -> ResultType<Option<FileTransferBlock>> {
        ...
        match &mut self.data_source {
            DataSource::FilePath(p) => {
                ...
                // Skip empty directory markers — no data to send
                if name.ends_with('/') {
                    self.file_num += 1;
                    self.file_confirmed = false;
                    self.file_is_waiting = false;
                    return Ok(Some(FileTransferBlock {
                        id: self.id,
        ...
        // Use larger chunks on direct (LAN) connections for better throughput;
        // keep conservative 64KB for relay connections to prevent memory issues.
        let buf_size: usize = if self.direct { 512 * 1024 } else { 64 * 1024 };
        ...
        // Resize buffer to actual data size to save memory
        ...
        if offset == 0 {
            ...
            // More conservative compression - only for larger blocks and with stricter savings requirement
            if matches!(self.data_source, DataSource::FilePath(_)) 
                && !is_compressed_file(name) 
                && offset > 16384 { // Only compress blocks larger than 16KB
                let tmp = compress(&buf);
                // Only use compression if it saves at least 20% to justify CPU cost
                if tmp.len() + (tmp.len() / 5) < buf.len() { 
                    buf = tmp;
                    compressed = true;
                }
            }
...
pub async fn handle_read_jobs(
    ...
    // Send up to max blocks per tick to improve throughput.
    // Use higher caps on direct (LAN) connections for better speed;
    // keep conservative values for relay connections.
    let max_blocks_per_tick: usize = if direct { 128 } else { 32 };
    let max_bytes_per_tick: usize = if direct { 16 * 1024 * 1024 } else { 2 * 1024 * 1024 };
    ...
    for job in jobs.iter_mut() {
        ...
        loop {
            match job.read(stream).await {
                ...
                Ok(Some(block)) => {
                    bytes_sent += block.data.len();
                    stream.send(&new_block(block)).await?;
                    blocks_sent += 1;
                    if blocks_sent >= max_blocks_per_tick || bytes_sent >= max_bytes_per_tick {
        ...
        // Still process one active job at a time, but send multiple blocks from it
        break;
    }

    // Immediate cleanup to free memory
    for id in finished {
        if let Some(mut job) = remove_job(id, jobs) {
            // Ensure streams are properly closed
            job.data_stream.take();
            drop(job);
        }
    }
